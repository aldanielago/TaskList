...
export function Input ({ type = 'text', item, value, name, setEdit, onChange, mainFunction, text = 'small', placeholder }) {
  ...
  return (
    <>
      {type === 'text' &&
        <input
          className={`${text == 'big' ? 'text-base' : 'text-sm'} pl-0 pb-1 dark:text-white font-Quicksand bg-inherit text-base max-w-[90%] md:max-w-md mb-2 border-0 border-b-2 border-gray-200 focus:ring-0 focus:border-pblue-500 transition-colors ease-linear delay-200 duration-150 form-input w-full`}
          name={name}
          value={value}
      ...
      {type == 'date' &&
        <input
          className='pl-0 pb-1 font-Quicksand bg-inherit text-sm max-w-md mb-2 border-0 border-b-2 border-gray-200 focus:ring-0 focus:border-pblue-500 transition-colors ease-linear delay-200 duration-150 form-input w-full'
          type='date'
          value={value}
