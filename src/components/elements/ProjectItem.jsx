...
export function ProjectItem({ project }) {
  ...
  return (
    <div className={`${projectsPalette[project.paletteId].primaryColor} max-w-[300px] min-w-[200px] p-3 pl-5 mt-2 w-11/12 rounded-3xl flex justify-between items-center shadow-md md:flex-col md:items-start md:rounded-lg`}>
      <i className={`${projectsPalette[project.paletteId].primaryColor == 'bg-light-green' ? "text-zinc-300" : "text-zinc-500"} text-3xl hidden md:block pb-[4px]`}>{ project.icon ? projectIcons[project.icon].component : projectIcons[0].component }</i>
