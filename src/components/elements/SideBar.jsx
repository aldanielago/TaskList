...
export function SideBar() {
  ...
  return (
    <nav className="bg-[#ceddfd] w-[20%] h-screen relative border-r border-inherit border-primary-blue border-opacity-5">
      ...
      <h3 className="font-Quicksand pt-2 mt-6 pl-8 block font-semibold">Projects</h3>
      <div>
        { projects.map((project) =>
          <Link to={`/projects/${project.id}`} key={project.id} className="w-full flex py-[10px] mt-6 pl-8 hover:bg-[#aac2f4] hover:border-[#aac2f4] transition-colors duration-300 active:border-l-4 border-l-4 focus:bg-[#aac2f4] focus:font-medium active:border-l-primary-blue focus:border-l-primary-blue focus:outline-none">
            ...
            <span className="font-Quicksand">{project.name}</span>
          </Link>
        )}
      </div>
    </nav>
